...
def clean_lookaheads(df,cols=['LONG_BL','SHORT_BL','LONGS_SCORE','SHORTS_SCORE'],N=10): # removes bl signal from adjacent points 
    ...
    # evaluates long short signals 
def lookahead_eval(df,long_entry='LONG_BL',short_entry='SHORT_BL',long_exit='SHORT_BL',short_exit='LONG_BL'): 
    long_portfolio = 100 
    long_amo=0
    short_portfolio = 100 
    short_amo=0
...
df[cols] =df.apply( lookahead_score, perc=0.0075, df=df,  N=25 ,axis = 1  ) 

amo=lookahead_eval(df)
print(amo)


#sers=[df['LONGS_SCORE'],df['SHORTS_SCORE']]
#u().plot_candles(df=df,sers=sers,longs_bl=df['LONG_BL'],shorts_bl=df['SHORT_BL'])
